...
# Writes config.sh which run.sh reads. Re-running setup.sh overwrites config.sh,
# but an existing API key is always preserved so clients keep working.
# All downloads and the toolbox build happen in a final fetch phase, *after*
# config.sh is written — a failed download never throws away your answers.
set -euo pipefail
...
# config.sh — Generated by setup.sh on $(date -Iseconds)
# Re-running setup.sh will overwrite this file (your API key is preserved).
